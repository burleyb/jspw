...
/**
 * Map all node-propertyware object to REPL context
 * @private
 */
Repl.prototype._defineBuiltinVars = function(context) {
  var cli = this._cli;

  // define propertyware package root objects
  for (var key in jspw) {
    if (jspw.hasOwnProperty(key) && !global[key]) {
      context[key] = jspw[key];
    }
  }
  // expose jspw package root object in context.
  context.jspw = jspw;
