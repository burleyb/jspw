/*global process */
/**
 * @file Command line interface for JSpw
...
function setLoginServer(loginServer) {
  ...
  if (loginServer === 'production') {
    defaultLoginUrl = 'https://login.propertyware.com';
  } else if (loginServer === 'sandbox') {
    defaultLoginUrl = 'https://test.propertyware.com';
...
function connect(name, options, callback) {
  ...
  conn = new jspw.Connection(connConfig);
...
function authorize(clientName, callback) {
  ...
  var oauth2 = new jspw.OAuth2(oauth2Config);
  ...
  waitCallback(oauth2Config.redirectUri, state, function(err, params) {
    if (err) { return callback(err); }
    conn = new jspw.Connection({ oauth2: oauth2 });
    if (!params.code) {
...
function register(clientName, clientConfig, callback) {
  ...
  var prompts = {
    ...
    "loginUrl": "Input login URL (default is https://login.propertyware.com) : "
  };
