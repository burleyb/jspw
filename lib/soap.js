...
var SOAP = module.exports = function(conn, options) {
  ...
  this._xmlns = options.xmlns || 'urn:service.web.propertyware.realpage.com';
};
...
function convertType(value, schema) {
  ...
  if (_.isArray(value)) {
    ...
    // console.log('------------------in single value section------------');
    if (_.isArray(schema)) {
      return [ convertType(value, schema[0]) ];
...
function toXML(name, value) {
  ...
  if (_.isArray(value)) {
    ...
    // var startTag = name ? '<ser:' + name + (attrs.length > 0 ? ' ' + attrs.join(' ') : '') + ' soapenv:encodingStyle="http://schmeas.xmlsoap.org/soap/encoding/">' : '';
    var startTag = name ? '<' + name + (attrs.length > 0 ? ' ' + attrs.join(' ') : '') + '>' : '';
    var endTag = name ? '</' + name + '>' : '';
    // if(value) {
    //   value = '<arg xsi:type="xsd:string">' + value + '</arg>';
    // }
